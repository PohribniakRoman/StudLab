...
export const PhotoInput:React.FC<any> = ({formData,withLoad=true,withLabel = true,defultAvatar = ""}) => {
    const [avatar,setAvatar] = useState<string>(defultAvatar);
    return(
        <label className="auth__form--avatar-wrapper">
            {withLoad?<input type="file" className="auth__form--file-input" accept=".jpg, .jpeg, .png" onChange={(event)=>{
